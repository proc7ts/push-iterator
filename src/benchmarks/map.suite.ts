...
export function *generatorMap<T, TResult>(
  source: Iterable<T>,
  map: (src: T) => TResult,
...
export function mapIterable<T, TResult>(
  source: Iterable<T>,
  map: (src: T) => TResult,
): Iterable<TResult> {
  return {
    [Symbol.iterator]() {
      ...
      return {
        next() {
          ...
          return next.done
            ? ({ done: true } as IteratorReturnResult<TResult>)
