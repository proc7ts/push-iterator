 ...
 */
export function thruIt<T, TReturn1>(
  it: Iterable<T>,
  pass1: (this: void, arg: T) => TReturn1,
): PushIterable<Out<TReturn1>>;

export function thruIt<T, TReturn1, TArgs2 extends Args<TReturn1>, TReturn2>(
  it: Iterable<T>,
  pass1: (this: void, arg: T) => TReturn1,
...
export function thruIt<T, TReturn>(
  ...
  const chain = (outcome: PushIterable<unknown>[], index: number): IterableCallChain => {
    ...
    return {
      ...
      iterate<TElement>(
        fn: (this: void, arg: TElement) => void,
        iterable: Iterable<TElement>,
      ): void {
        outcome.push(
          flatMapIt(iterable, item => {
